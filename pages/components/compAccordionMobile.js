...
export default function CompAccordionMobile({ data = {}, style = 'white' }) {
    ...
    return (
        <>
            ...
            <div className="container-imgs lg:py-12 md:py-12 py-12" >
                ...
                <div className={`gallery-wrap`}>
                    {data.accordion_items?.map((item, i) => (
                        <div className="item-mobile item-1 relative" id={item.comp_accordion_items_id?.subtitle} style={{ backgroundImage: `url(${image_url}${item.comp_accordion_items_id?.image?.id})` }}
                            >
                            ...
                            <div className='' >
                                {/* {selected === i && ( */}
                                    <div  className="absolute bottom-2 left-16 right-16" >
                                        ...
                                        {item.comp_accordion_items_id?.brief ? <p className="text-[#D8D8D8] futura-book text-xl mt-2" > {item.comp_accordion_items_id?.brief}  </p> : null}
                                        {item.comp_accordion_items_id?.button_url?<a href={`${item.comp_accordion_items_id?.button_url}`}  className="futura-bold">{item.comp_accordion_items_id?.button_title}<ChevronRightIcon /></a>:null}
            ...
            {data.show_classes ?<div className=''>
                <div className={`mb-20 container  mx-auto pt-10 relative `}>
                    {ListofClasses.filter((item) => item.value === selectedCategory).map((element, index) => (
                        <div className="" id="classes">
                        <div className='lg:flex mb-20 mx-auto items-center'>
