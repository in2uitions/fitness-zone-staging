...
export default function App() {
                    ...
                    try {
                        ...
                        if (response.status === 200) {
                            ...
                            handleCategoryChange({
                                target: { value: locationValue.locationCode },
                            });
                        } else {
                            setLocation([]);
                        }
                ...
                if (locationCode.country_code === "LB") {
                    try {
                        ...
                        if (locationValue && locationValue.locationCode) {
                            query =
                                query +
                                `${query !== "" ? "&" : "?"}LocationCode=${locationValue.locationCode
                                }`;
                        }
                        try {
                            var registrationLoginHeaders = new Headers();
    ...
    function handleClassChange(event) {
        if (event.target.id == "All") {
            ...
            let newvalue = data.filter((item) =>
                handleClassChangeWithSearch(item, event.target.id, name)
            );
    ...
    function handleClassChangeWithSearch(item, valuename, value) {
        if (value != "" && valuename != "") {
            return (
                item.studio.studioName === valuename &&
                `${item.class?.className} ${item.studio?.studioName} ${item.location?.locationName
                    } ${moment(item.classTime).format("DD MMM YYYY")} ${moment(
                        item.classTime
                    ).format("HH:mm")}`
                    .toLowerCase()
            ...
            return `${item.class?.className} ${item.studio?.studioName} ${item.location?.locationName
                } ${moment(item.classTime).format("DD MMM YYYY")} ${moment(
                    item.classTime
                ).format("HH:mm")}`
                .toLowerCase()
                ...
                >
                            ...
                            <div className="flex justify-center items-center">
                                ...
                                >
                                    {location.map((item, i) => (
                                        <>
                                            {item.isActive ? (
                                                <option key={i} value={item.locationCode} id="location">
                                                    {item.locationName}
                                                </option>
                                            ) : null}
                                        </>
                        ...
                        >
                            ...
                            <div className="relative" style={{ width: "35%" }}>
                                <input
                                    ...
                                    name="search"
                                    id="search"
                                    className="w-full border border-gray-500 rounded-lg h-10 mt-5 mb-5 bg-transparent pl-4 input-search"
                                    placeholder="Search"
                                    value={name}
                                    onChange={handleSearch}
                                />
                            ...
                            <div className="flex justify-center items-center">
                                <select
                                    style={{ height: "2.5rem", borderRadius: "5px", background:"#3e3c3c", color:"white", borderColor:"#3e3c3c" }}
                                    name="location"
                                    id="location"
                                    onChange={handleCategoryChange}
                                >
                    ...
                    <BrowserView>
                        <Tabs className="" style={{ marginTop: "1.25rem" }}>
                            <TabList
                                className="flex justify-between w-full mx-auto container tabs-container"
                                style={{
                                    display: "flex",
                                    justifyContent: "space-between",
                                    width: "100%",
                            ...
                            >
                                <Tab className="notSelected cursor-pointer" id="All">
                                    <div
                                        className="flex items-center lg:space-x-2 md:space-x-2 space-x-1"
                                        style={{
                                            display: "flex",
                                            alignItems: "center",
                                            gap: "5px",
                                    ...
                                    >
                                        <p
                                            className="lg:text-2xl md:text-2xl text-xs font-extrabold"
                                ...
                                <Tab className="notSelected cursor-pointer">
                                    <div
                                        className="flex justify-end items-center lg:space-x-2 md:space-x-2 space-x-1"
                                        style={{
                                            display: "flex",
                                            alignItems: "center",
                                            gap: "5px",
                    ...
                    <MobileView>
                        <Tabs className="" style={{ marginTop: "1.25rem" }}>
                            ...
                            style={{display:"grid", padding:"0px"}}>
                                <Tab
                                    title="nested"
                                    className="notSelected cursor-pointer"
                                    id="All"
                                >
                                    <div
                                    style={{display:"flex", alignItems:"center", gap:"5px"}}
                                        ...
                                        id="All"
                                    >
                                        <p className="text-base font-extrabold" id="All">
                                            All
                                        </p>
                                        <img src="/ONblue.png" className="on-tabs" id="All" />{" "}
                                    </div>
                                ...
                                <Tab className="notSelected cursor-pointer">
                                    ...
                                    style={{display:"flex", alignItems:"center", gap:"5px"}}
                                        ...
                                        type="button"
                                    >
                                        <p className="text-base font-extrabold" id="NRG">
                                            ENERGY
                                        </p>
                                        <img src="/ONblue.png" className="on-tabs" id="NRG" />{" "}
                                    </div>
                                ...
                                <Tab className="notSelected cursor-pointer">
                                    ...
                                    style={{display:"flex", alignItems:"center", gap:"5px"}}
                                        ...
                                        type="button"
                                    >
                                        <p className="text-base font-extrabold" id="BLNC">
                                            BALANCE
                                        </p>
                                        <img src="/ONblue.png" className="on-tabs" id="BLNC" />{" "}
                                    </div>
                                ...
                                <Tab className="notSelected cursor-pointer">
                                    ...
                                    style={{display:"flex", alignItems:"center", gap:"5px"}}
                                        ...
                                        type="button"
                                    >
                                        <p className="text-base font-extrabold" id="PWR">
                                            POWER
                                        </p>
                                        <img src="/ONblue.png" className="on-tabs" id="PWR" />{" "}
                                    </div>
                                ...
                                <Tab className="notSelected cursor-pointer">
                                    ...
                                    style={{display:"flex", alignItems:"center", gap:"5px"}}
                                        ...
                                        type="button"
                                    >
                                        <p className="text-base font-extrabold" id="POOL">
                                            POOL
                                        </p>
                                        <img src="/ONblue.png" className="on-tabs" id="POOL" />{" "}
                                    </div>
                                ...
                                <Tab className="notSelected cursor-pointer">
                                    ...
                                    style={{display:"flex", alignItems:"center", gap:"5px"}}
                                        ...
                                        type="button"
                                    >
                                        <p className="text-base font-extrabold" id="Tennis Indoor">
                                            TENNIS
                                        </p>
                                        <img
                                            src="/ONblue.png"
                    ...
                    <BrowserView>
                        {filtered
                            ...
                            .map((item, index) => (
                                <>
                                    {moment(item.classTime).format("DD MMM YYYY HH:mm") >=
                                        todayTime ? (
                                        <div
                                            className="flex justify-between w-full classes-box mb-3 mt-10 p-3 flex-wrap"
                                            style={{
                                                display: "flex",
                                                justifyContent: "space-between",
                                                width: "100%",
                        ...
                        {filtered.map((item, index) => (
                            <>
                                {moment(item.classTime).format("DD MMM YYYY HH:mm") >=
                                    todayTime ? (
                                    <div
                                        className="flex justify-between w-full classes-box mb-3 mt-10 p-3"
                                        key={index}
                                    >
